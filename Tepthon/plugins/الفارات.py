...
""" وصـف الملـف : اوامـر إضافـة الفـارات باللغـة العربيـة كـاملة ولا حـرف انكلـش🤘 تخمـط اذكـر المصـدر يولـد
إضافـة فـارات صـورة ( الحمايـة - الفحـص - الوقتـي ) بـ امـر واحـد فقـط
حقوق : @zthon
...
ZelzalVP_cmd = (
    "𓆩 [surs alzaeim  𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر الفـارات](t.me/VVV5P) 𓆪\n\n"
    ...
    "⪼ `.اضف صورة البوت` بالـرد ع صـورة او ميديـا لـ إضافـة صـورة ستـارت للبـوت\n\n"
    ...
    "⪼ `.وقت المغرب` \n\n"
    "⪼ `.وقت تركيا` \n\n"
    "🛃 سيتـم إضافـة المزيـد من الدول قريبًـا\n\n"
    "\n𓆩 [𝙑𝙖𝙧𝙨 - قنـاة الفـارات](t.me/VVV5P) 𓆪"
)
...
async def variable(event):
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        if gvarstatus("ALIVE_TEMPLATE") is None:
            ...
            await zed.edit("**⎉╎تم إضـافــة {} بنجـاح ☑️**\n**⎉╎الكليشـة المضـافه** \n {} \n\n**⎉╎الآن قـم بـ ارسـال الامـر ↶** `.فحص` **لـ التحقـق مـن الكليشـة . .**".format(input_str, vinfo))
        ...
        if variable in heroku_var:
            await zed.edit("**⎉╎تم تغييـر {} بنجـاح ☑️**\n**⎉╎المتغيـر : ↶**\n `{}` \n**⎉╎يتم الآن إعـادة تشغيـل بـوت تيبثــون يستغـرق الأمـر 2-1 دقيقـة ▬▭ ...**".format(input_str, vinfo))
        else:
            await zed.edit("**⎉╎تم إضافـة {} بنجـاح ☑️** \n**⎉╎المضاف اليه :**\n `{}` \n**⎉╎يتم الآن إعـادة تشغيـل بـوت تيبثــون يستغـرق الأمـر 2-1 دقيقـة ▬▭ ...**".format(input_str, vinfo))
...
async def variable(event):
    ...
    viran = "Asia/Tehran"
    heroku_var = app.config()
    zed = await edit_or_reply(event, "**⎉╎جـاري إعـداد المنطقـة الزمنيــة لـ تيبثــون 🌐...**")
    # All Rights Reserved for "Zed-Thon" "زلـزال الهيبـه"
    if input_str == "العراق" or input_str == "اليمن" or input_str == "سوريا" or input_str == "السعودية" or input_str == "لبنان" or input_str == "الاردن":
        variable = "TZ"
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("**⎉╎تم تغييـر المنطقـة الزمنيـة .. بنجـاح ☑️**\n**⎉╎المتغير : ↶**\n دولـة `{}` \n**⎉╎يتم الآن إعـادة تشغيـل بـوت تيبثــون يستغـرق الأمـر 2-1 دقيقـة ▬▭ ...**".format(input_str))
        else:
            await zed.edit("**⎉╎تم إضـافــة المنطقـة الزمنيـة .. بنجـاح ☑️**\n**⎉╎المضـاف إليـه : ↶**\n دولـة `{}` \n**⎉╎يتم الآن إعـادة تشغيـل بـوت تيبثــون يستغـرق الأمـر 2-1 دقيقـة ▬▭ ...**".format(input_str))
