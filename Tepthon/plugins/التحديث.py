...
async def update_bot(event, repo, ups_rem, ac_br):
    ...
    sandy = await event.edit(f"ᯓ 
    𝐴𝐿𝑍𝐴𝐸𝐼𝑀   - تحـديثـات السـورس\n**•─────────────────•**\n\n**•⎆┊تم التحـديث ⎌ بنجـاح**\n**•⎆┊جـارِ إعـادة تشغيـل بـوت الـزعيــم ⎋ **\n**•⎆┊انتظـࢪ مـن 2 - 1 دقيقـه . . .📟**")
    await event.client.reload(sandy)


async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is None:
        return await event.edit(f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝐴𝐿𝑍𝐴𝐸𝐼𝑀  - تحـديثـات السـورس\n **•─────────────────•**\n** ⪼ لم تقـم بوضـع مربـع فـار HEROKU_API_KEY اثنـاء التنصيب وهـذا خطـأ .. قم بضبـط المتغيـر أولاً لتحديث بوت الـزعيــم ..؟!**", link_preview=False)
    heroku = heroku3.from_key(HEROKU_API_KEY)
    heroku_applications = heroku.apps()
    if HEROKU_APP_NAME is None:
        await event.edit(f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝐴𝐿𝑍𝐴𝐸𝐼𝑀  - تحـديثـات السـورس\n **•─────────────────•**\n** ⪼ لم تقـم بوضـع مربـع فـار HEROKU_APP_NAME اثنـاء التنصيب وهـذا خطـأ .. قم بضبـط المتغيـر أولاً لتحديث بوت الـزعيــم ..؟!**", link_preview=False)
        repo.__del__()
        return
    heroku_app = next(
    ...
    if heroku_app is None:
        await event.edit(
            f"{txt}\n" "**- بيانات اعتماد هيروكو غير صالحة لتنصيب تحديث الـزعيــم**"
        )
        return repo.__del__()
    sandy = await event.edit(f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝐴𝐿𝑍𝐴𝐸𝐼𝑀  - تحـديثـات السـورس\n**•─────────────────•**\n\n**✾╎جـارِ . . تنصـيب التحـديث الجـذري ⎌**\n**✾╎يـرجى الانتظـار حتى تنتـهي العمليـة ⎋**\n**✾╎عادة ما يستغرق هـذا التحديث من 5 - 4 دقائـق 📟**")
    try:
...
async def upstream(event):
    ...
    event = await edit_or_reply(event, f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝐴𝐿𝑍𝐴𝐸𝐼𝑀  - تحـديثـات السـورس\n**•─────────────────•**\n**⪼ جاري البحث عن التحديثات  🌐.. ،**")
    off_repo = UPSTREAM_REPO_URL
    force_update = False
    if ENV and (HEROKU_API_KEY is None or HEROKU_APP_NAME is None):
        return await edit_or_reply(
            event, f"ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝐴𝐿𝑍𝐴𝐸𝐼𝑀  - تحـديثـات السـورس\n**•─────────────────•**\n** ⪼ اضبط الفـارات المطلوبة أولاً لتحديث بوت الـزعيــم ،**"
        )
    try:
        txt = (
